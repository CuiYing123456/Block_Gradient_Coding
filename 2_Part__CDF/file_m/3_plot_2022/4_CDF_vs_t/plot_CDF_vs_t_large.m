...
%% system para (large t)
t_axis = 10 .^ [6.50:0.05:6.70]; num_points = length(t_axis);
N = 20; L = 4e4; mu = 10^(-1); t_0 = 1; M = 50; b = 1; num_sample = 1e5;
%%
iter_Max = 200;
%% 
...
for i = 1:num_points
    x_get_int = fun_cal_sta_point(t_axis(i) , L, M, N, b, mu, t_0 , iter_Max);
    L_prop_sta_point(i)             = fun_Est_CDF_T(t_axis(i), x_get_int, T_mat, M, N, b);
    L_prop_approx_lg(i)             = fun_Est_CDF_T(t_axis(i), x_large, T_mat, M, N, b);
    ...
    L_Draper_r_L(i)                 = fun_B3_Draper_v3(t_axis(i), L, T_mat, M, N, b, 1);
    L_Draper_r_L_D_2(i)             = fun_B3_Draper_v3(t_axis(i), L, T_mat, M, N, b, 2);
end
...
%%
figure(1);
line_L_prop_sta_point = semilogx(t_axis,L_prop_sta_point);     hold on;
line_L_prop_approx_lg = semilogx(t_axis,L_prop_approx_lg);     hold on;

line_L_Single_BCGC    = semilogx(t_axis,L_Single_BCGC);        hold on;
line_L_Tandon         = semilogx(t_axis,L_Tandon);             hold on;
line_L_Draper_r_L     = semilogx(t_axis,L_Draper_r_L);         hold on;
line_L_Draper_r_L_D_2 = semilogx(t_axis,L_Draper_r_L_D_2);     hold on;
...
line_L = [line_L_prop_sta_point , line_L_prop_approx_lg , ...
...
set(gca , 'XLim' , [10^(6.40),10^(6.70)] , 'XTick' , 10.^[6.40:0.05:6.70] , 'FontSize' , 20);
