...
mu_axis = 10 .^ [-1:0.1:-0.5]; num_points = length(mu_axis);

N = 20; L = 4e4; t_0 = 1; t = 10 .^ (6.5);  M = 50; b = 1; num_sample = 1e5;
%%
iter_Max = 200;
%% approx. solutions
...
for i = 1:num_points    
    ...
    T_mat = exprnd(1/mu_axis(i),[num_sample,N]) + t_0;
%     x_get_int = fun_cal_sta_point(t , L, M, N, b, mu_axis(i), t_0 , iter_Max);
%     L_prop_sta_point(i)             = fun_Est_CDF_T(t, x_get_int, T_mat, M, N, b);
    L_prop_approx_lg(i)             = fun_Est_CDF_T(t, x_large, T_mat, M, N, b);
%     [L_Single_BCGC(i),B1_index(i)]  = fun_B1_Single_BCGC_v3(t, L, T_mat, M, N, b);
%     [L_Tandon(i),B2_index(i)]       = fun_B2_Tandon_v3(t, L, T_mat, M, N, b);
%     L_Draper_r_L(i)                 = fun_B3_Draper_v3(t, L, T_mat, M, N, b, 1);
%     L_Draper_r_L_D_2(i)             = fun_B3_Draper_v3(t, L, T_mat, M, N, b, 2);
    
    %% Plot point by point
    line_L_prop_sta_point = semilogx(mu_axis(i),L_prop_sta_point(i));     hold on;
    line_L_prop_approx_lg = semilogx(mu_axis(i),L_prop_approx_lg(i));     hold on;
    line_L_Single_BCGC    = semilogx(mu_axis(i),L_Single_BCGC(i));        hold on;
    line_L_Tandon         = semilogx(mu_axis(i),L_Tandon(i));             hold on;
    line_L_Draper_r_L     = semilogx(mu_axis(i),L_Draper_r_L(i));         hold on;
    line_L_Draper_r_L_D_2 = semilogx(mu_axis(i),L_Draper_r_L_D_2(i));     hold on;
    ...
    set(gca , 'XLim' , [10^(-1),10^(-0.5)] , 'XTick' , 10 .^ [-2:0.2:-1] , 'FontSize' , 20);
