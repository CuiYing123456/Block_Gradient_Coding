%% Change y with w: w_n = mutN__Mb * y_n
f_CDF = fopen('proposed_solution.txt','wt');
% for t = 10 .^ [7.75]
% for L = [1 2 3 4 5] * 10^(4)
% for mu = 10 .^ [-2.8 -2.6]
for t = 10^(6.5)
    
tic;
% clear all; 
clearvars -except f_CDF t;
clc;
%%
...
% ================================================
N = 30; L = 5750; 
mu = 5*10^(-2); M = 50; b = 1; t = 10^(6.5); t_0 = 0; num_sample = 1e4;
% ================================================
...
%%
% variable_new = ones(2*N,1);
% variable_old = zeros(2*N,1);
%% set initial points
...
% while 1
%     tic;
%     CDF = fun_large_ln_estimate_CDF_par(u_i,N,num_Max,B,12);
%     fprintf(2,['CDF = ', num2str(CDF), '\n']);
%     toc;
% 
%     CDF_init = fun_large_ln_estimate_CDF_par(u_init,N,num_Max,B,12);
%     fprintf(2,['CDF_init = ', num2str(CDF_init), '\n']);
% %% Calculate x
%     w_get = w_i; u_get = u_i;
%     y_get = w_i / mutN__Mb; x_get =  (1./y_get - [0;1./y_get(1:N-1)] ) ./ [1:N]';
%     if CDF > 0.65 && CDF < 1
%         break;
%     end
% end
% fprintf(2,['CDF_final = ', num2str(CDF), '\n']);
...
% ================================================v
CDF_est_proposed_real = fun_Est_CDF_real_samples(t,x,num_sample,  N,M,b);
fprintf(2,['CDF_est_proposed_real = ', num2str(CDF_est_proposed_real), '\n']);
% ================================================^
%%
fclose(fid);
%% 
% fprintf(f_CDF,'N = %d,\t CDF_est = %f\n',N,CDF_est_proposed);
fprintf(f_CDF,'t = %d,\t CDF_est = %f\n',t,CDF_est_proposed);
% fprintf(f_CDF,'mu = %d,\t CDF_est = %f\n',mu,CDF_est_proposed);
% fprintf(f_CDF,'L= %d,\t CDF_est = %f\n',L,CDF_est_proposed);
%% para backup

% N = 50; L = 1000;
% mu = 1e-2; M = 50; b = 1; t = 5e5; t_0 = 1; % 'MbN'= M*b/N
% 
% mutN__Mb = mu * t * N / (M * b);
% A = exp(-mutN__Mb); B = exp(mu * t_0);
% I = eye(N); %for extracting unit vector e_i
